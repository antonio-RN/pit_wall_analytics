...
tab_Home.markdown("Raw data received from _ff1.get_event_schedule()_ -> ff1.events.EventSchedule")
tab_Home.dataframe(select_event_schedule)
...
tab_Home.markdown("Clean data accessed from _ff1.core.events.EventSchedule_ -> ff1.events.Event")
tab_Home.dataframe(next_event)
...
tab_Results.markdown("Call to _ff1.get_session()_ -> ff1.core.Session")
tab_Results.markdown("Raw data from ff1.core.Session.results -> ff1.core.Results")
tab_Results.write(select_session.results)
...
tab_Results.markdown("Fuel-correction calculation per lap")
tab_Results.dataframe(df_fuel_correction)
...
tab_Results.markdown("Color scheme used for general data visualization")
tab_Results.dataframe(df_color_schema)
...
tab_Results.markdown("Call to _ff1.core.Session.load_data(laps=True)_ -> f1.core.Laps")
if ((st.session_state.sel_GP_session == "Race") | (st.session_state.sel_GP_session =="Sprint")):
    tab_Results.markdown("Clean data for Race _Race position_")
    tab_Results.dataframe(df_laps_position)
else:
    tab_Results.markdown("Clean data for Qualifying _Gap to best time (s)_")
    tab_Results.dataframe(df_best_laps)

tab_Results.markdown("Clean data for _Lap time distribution (s)_")
tab_Results.dataframe(df_total_laps)
...
# Driver #1
    ...
    tab_Laps.markdown("Raw data from _ff1.core.Session.load_data(laps=True)_ -> f1.core.Laps")
    tab_Laps.dataframe(select_laps_1)
...
# Data formatting 
    ...
    tab_Laps.markdown("Clean data for _Lap times per stint (s)_ and _Lap time distirbution (s)_")
    tab_Laps.dataframe(df_select_laps)
...
def show_metrics_lap_2():
    ...
    with colT2:
        ...
        # )
if len(list_laps_selection)>0:
    tab_Telemetry.markdown("Raw data from _ff1.core.Session.load_data(telemetry=True)_ -> f1.core.Telemetry")
    tab_Telemetry.dataframe(df_telemetry_laps)
    ...
    tab_Telemetry.markdown("Clean data for vehicle channels visualization and _Vehicle speed (km/h)_")
    tab_Telemetry.dataframe(df_Telemetry)
...
# Calculate fastest driver per minisectors (only for 2 laps selected)
    if len(list_laps_selection)>1:
        ...
        tab_Telemetry.markdown("Clean data for _Lap dominance per minisector_")
        tab_Telemetry.dataframe(df_telemetry_laps_inter)
        tab_Telemetry.divider()
